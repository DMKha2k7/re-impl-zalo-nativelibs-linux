...
console.log('🏁 Building file-utils...');
...
try {
  ...
  const destWrapper = path.join(DEST_DIR, 'file-utils', 'index.js');
  patchFile('file-utils/index.js', destWrapper);
} catch (e) {
...
const srcBinary = path.join(LIB_DIR, 'build', 'Release', 'file-utils.node');
if (!fs.existsSync(srcBinary)) {
  throw new Error(`Compiled addon not found: ${srcBinary}`);
}
const destBinary = path.join(DEST_DIR, 'file-utils', 'linux', 'file-utils.node');
...
console.log('✅ file-utils build completed.');
